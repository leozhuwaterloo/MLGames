...
var _BlockAnimation = __webpack_require__(!(function webpackMissingModule() { var e = new Error("Cannot find module \"BlockAnimation.js\""); e.code = 'MODULE_NOT_FOUND'; throw e; }()));
  ...
  _createClass(App, [{
    ...
    value: function sketch(p) {
      ...
      p.draw = function () {
        ...
        for (var z = 0; z < p.height; z += w) {
          for (var x = 0; x < p.width; x += w) {
            p.push();
            var d = p.dist(x, z, p.width / 2, p.height / 2);
            var offset = p.map(d, 0, maxD, -p.PI, p.PI);
            var a = angle + offset;
            var h = p.floor(p.map(p.sin(a), -1, 1, 100, 350));
    ...
    value: function render() {
      return _react2.default.createElement(
        'div',
        null,
        _react2.default.createElement(P5Wrapper, { sketch: this.sketch })
      );
    }
  }]);
